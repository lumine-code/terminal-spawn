...
function pathForEvent(event) {
  ...
  const editorPath = target?.closest?.("atom-text-editor:not([mini])")?.getModel?.()?.getPath?.();
...
module.exports = {
  ...
  activate() {
    this.disposables = new CompositeDisposable(
      atom.commands.add("atom-workspace", {
        "terminal-spawn:open": (event) => module.exports.openTerminal(pathForEvent(event)),
        "terminal-spawn:root": () => module.exports.openTerminal(),
        "terminal-spawn:list": () => selectPreset.show(),
      }),
    );
  },
  ...
  openTerminal(filepath, command) {
    ...
    if (!dirpath) {
      atom.notifications.addWarning("terminal-spawn: no directory to open", {
